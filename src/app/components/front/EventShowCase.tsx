        ...
        <div className="grid md:grid-cols-3 gap-8 border-black">
          {latest.map((event) => (
            <Link
              href={`/event/${event.id}`}
              key={event.id}
              className="block rounded-xl overflow-hidden shadow-lg border-2 hover:scale-[1.01] transition border-blue-600"
            >
              <img
              ...
              />
              <div className="p-4">
                <h3 className="font-bold text-gray-100text-lg mb-1">{event.name}</h3>

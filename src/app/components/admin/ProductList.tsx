    ...
    <table className="min-w-[900px] w-full text-sm border border-gray-300">
      <thead>
        <tr className="bg-gray-200 text-gray-900">
          ...
          <th className="p-2 border border-gray-300">VIP</th>
          <th className="p-2 border border-gray-300">Gold</th>
          <th className="p-2 border border-gray-300">Silver</th>
          <th className="p-2 border border-gray-300">Bronze</th>
        ...
        {products.map((product) => (
          <tr key={product.id} className="border-t border-gray-300">
            {editingId === product.id ? (
              ...
              <>
                <td className="p-2 border font-medium text-gray-900">{product.name}</td>
                <td className="p-2 border text-gray-800">{product.description}</td>
                <td className="p-2 border">
                  <span className="bg-gray-100 px-2 py-1 rounded text-sm text-black">
                    £{product.price_v.toFixed(2)}
                  </span>
                </td>
                <td className="p-2 border">
                  <span className="bg-yellow-100 px-2 py-1 rounded text-sm text-black">
                    £{product.price_g.toFixed(2)}
                  </span>
                </td>
                <td className="p-2 border">
                  <span className="bg-blue-100 px-2 py-1 rounded text-sm text-black">
                    £{product.price_s.toFixed(2)}
                  </span>
                </td>
                <td className="p-2 border">
                  <span className="bg-amber-100 px-2 py-1 rounded text-sm text-black">
                    £{product.price_b.toFixed(2)}
                  </span>
                </td>
                <td className="p-2 border">
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="px-2 py-1 bg-red-100 hover:bg-red-200 rounded text-black"
                    >
                      -
                    </button>
                    ...
                    <button
                      onClick={() => adjustQuantity(product.id, 10)}
                      className="px-2 py-1 bg-green-100 hover:bg-green-200 rounded text-black"
                    >
                      +
                    </button>
                  </div>
